...
public interface CustomerRepository extends JpaRepository<Customer,Long> {
    @Query("SELECT c FROM Customer c WHERE c.deletedAt IS NULL AND c.active = :active")
    Page<Customer> findAllByActive(@Param("active") Boolean active, Pageable pageable);
    ...
    @Query(value = "SELECT * FROM customers WHERE deleted_at IS NOT NULL", nativeQuery = true)
    Page<Customer> findAllDeleted(Pageable pageable);

    // Fetch a specific deleted customer by ID (bypasses @Where)
    @Query(value = "SELECT * FROM customers WHERE id = :id AND deleted_at IS NOT NULL", nativeQuery = true)
